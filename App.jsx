...
function Projects({ go }) {
  ...
  async function create() {
    if (!nm.trim()) return
    setSS('saving')
    const { data, error } = await sb.from('projects').insert({ name:nm.trim(), desc:ds.trim(), color:col, tags:DEFAULT_TAGS }).select().single()
          ...
          {(list||[]).map((p,i) => (
            <div key={p.id} style={{ animation:`up .25s ease ${i*.04}s both` }}>
              ...
              onMouseLeave={e => { e.currentTarget.style.transform='none'; e.currentTarget.style.borderColor='#0F172A'; e.currentTarget.style.borderTopColor=p.color }}>
                ...
                {p.desc && <div style={{ color:'#475569', fontSize:12, lineHeight:1.5 }}>{p.desc}</div>}
...
function NewItemModal({ tags: initTags, onAddTag, onSave, color, onClose }) {
  ...
  const [form, setForm]    = useState({ title:'', status:'backlog', prio:'medio', tags:[], desc:'', qa:'', dev:'', media:[] })
      ...
      <div style={{ paddingTop:18 }}>
        {tab === 'info' && (
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div><label style={lbl}>Descrição</label><textarea value={form.desc} onChange={e => f('desc',e.target.value)} placeholder="Detalhe o problema..." rows={4} style={{...inp,resize:'vertical',fontFamily:'inherit'}}/></div>
          </div>
        )}
      ...
      <div style={{ maxWidth:900, margin:'0 auto', padding:'26px 22px' }}>
        ...
        {item.desc && (
          <div style={{ marginBottom:18 }}>
            ...
            <div style={{ background:'#080D18', border:'1px solid #0F172A', borderRadius:8, padding:14, color:'#94A3B8', fontSize:13, lineHeight:1.75 }}>{item.desc}</div>
          </div>
        )}
